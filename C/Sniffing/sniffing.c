	...
	switch(ptl_num) {
		case 1: // ICMP 패킷만 출력 
			while(1) {
				...
				// recvfrom 함수가 실패할 경우 continue 
				if(ret<0) continue;
				// recvfrom 함수가 성공할 경우 ICMP 패킷만 출력 
				if((ip->protocol == PROTO_ICMP)) {
					ip_header_print(ip, ret, argv[1]);
					icmp_print(ip, buffer);
				}
				// 수신할 때 너무 느리지 않게 하기 위해 0.5초마다 수신할 수 있게함 
				sleep(0.5);
			}
			break;
		case 6: // TCP 패킷만 출력
			while(1) {
				...
				// recvfrom 함수가 실패할 경우 continue 
				if(ret<0) continue;
				// recvfrom 함수가 성공할 경우 TCP 패킷만 출력
				if((ip->protocol == PROTO_TCP)) {
					ip_header_print(ip, ret, argv[1]);
					tcp_print(ip, buffer);
				}
				// 수신할 때 너무 빠르지 않게 1초마다 수신할 수 있게함 
				sleep(1);
			}
			break;
		case 17: // UDP 패킷만 출력 
			while(1) {
				...
				// recvfrom 함수가 실패할 경우 continue 
				if(ret<0) continue;
				// recvfrom 함수가 성공할 경우 UDP 패킷만 출력 
				if((ip->protocol==PROTO_UDP)) {
					ip_header_print(ip, ret, argv[1]);
					udp_print(ip, buffer);
				}
				// 수신할 때 너무 느리지 않게 0.5초마다 수신할 수 있게함 
				sleep(0.5);
			}
			break;
		case 0: // icmp, tcp, udp의 패킷들 전부 출력 
			while(1) {
				// recvfrom 함수가 호출되기 전에 memset 함수를 통해 buffer의 크기만큼 0x00으로 buffer 초기화
				memset(buffer, 0x00, sizeof(buffer));
				// 배열 buffer의 크기만큼 nonblocking으로 수신함 
				...
				if(ret<0) continue;
				// recvfrom함수가 성공할 경우 icmp, tcp, udp 패킷들에 대하여 출력하고
				// 만약 icmp, tcp, udp 패킷이 아니라면 continue 
				if((ip->protocol == PROTO_ICMP)) {
					ip_header_print(ip, ret, argv[1]);
					icmp_print(ip, buffer);
				}
